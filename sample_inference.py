...
# rgb = np.array(Image.open('datasets/transcg/transcg/scene3/1/rgb1.png'), dtype = np.float32)

# rgb = np.array(Image.open('datasets/transcg/transcg/scene3/1/rgb1.png')
rgb = cv2.imread('datasets/transcg/transcg/scene28/20/rgb1.png')
rgb_mask = np.array(Image.open('datasets/transcg/transcg/scene28/20/depth1-gt-mask.png'), dtype = np.float32)
depth = np.array(Image.open('datasets/transcg/transcg/scene28/20/depth1.png'), dtype = np.float32)
depth_gt = np.array(Image.open('datasets/transcg/transcg/scene28/20/depth1-gt.png'), dtype = np.float32)
...
# res,ori_depth = inferencer.inference(rgb, depth)

res = inferencer.inference(rgb, depth)
res = res*255
cv2.imwrite('transcg_28_20_TDC_pred.png',res)
cam_intrinsics = np.load('datasets/transcg/transcg/camera_intrinsics/1-camIntrinsics-D435.npy')
...
ori_depth = depth*255
ori_depth= ori_depth.astype(np.int)
depth_gt= depth_gt*255
depth_gt= depth_gt.astype(np.int)
...
axs.flat[5].imshow(error,vmax=1,vmin=0,cmap=tt)
axs.flat[5].set_title("masked_res")

sc = plt.imshow(error, vmax=1,vmin=0,cmap = plt.cm.jet)# 限制范围为0-100
plt.colorbar()
...
# cloud = draw_point_cloud(rgb_1, res, cam_intrinsics, scale = 1.0)
# cloud_gt = draw_point_cloud(rgb_1, depth_gt, cam_intrinsics, scale = 1.0)
#
...
# o3d.visualization.draw_geometries([cloud_gt, frame, sphere])
